...
 */
struct capability_info procbased_sec[27] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Pinbased Controls MSR info: 0x%llx\n",
	...
	report_capability(pinbased, 5, lo, hi);
        ...
        printk(KERN_INFO "Procbased Primary\n");

	rdmsr(IA32_VMX_PROCBASED_CTLS, lo, hi);
	pr_info("Procbased Controls MSR info: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased, 21, lo, hi);
        ...
        printk(KERN_INFO "Procbased Secondary\n");

	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Procbased secondary Controls MSR info: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased_sec, 27, lo, hi);

        
        /* Exit control */
        printk(KERN_INFO "Exit Control\n");

	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Exit Controls MSR info: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vm_exit, 14, lo, hi);


        /* Entry control */
        printk(KERN_INFO "Entry Control\n");

	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Control MSR info: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
